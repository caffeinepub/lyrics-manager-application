...
import type { ColorRange, SetList, Song } from "../backend";
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["setlist", variables.id, "songs"],
      });
    },
    onError: () => {
...
export function useAddSongToSetList() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["setlist", variables.setListId, "songs"],
      });
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (data.direction === "up" && currentIndex > 0) {
        ...
        data.direction === "down" &&
        currentIndex < setList.songIds.length - 1
      ) {
        newPosition = currentIndex + 1;
      } else {
        // No movement needed
      ...
      [newSongIds[currentIndex], newSongIds[newPosition]] = [
        newSongIds[newPosition],
...
export function useImportData() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success(
        `Data imported successfully! Restored ${songCount} song${songCount !== 1 ? "s" : ""} and ${setListCount} set list${setListCount !== 1 ? "s" : ""}.`,
      );
    },
    onError: (error) => {
...
export function useDownloadBackup() {
  ...
  return useMutation({
    ...
    onSuccess: async (exportData) => {
      ...
      if (result.success) {
        toast.success(
          `Backup saved to ${result.path || "Lyrics Manager.json"}`,
        );
      }
    },
    onError: () => {
  ...
  return useMutation({
    mutationFn: async (songFiles: SongFileData[]) => {
      ...
      for (const songData of songFiles) {
        ...
        const colorRanges: ColorRange[] = (songData.colorRanges || []).map(
          (r) => ({
    ...
    onSuccess: (importedSongs) => {
      ...
      toast.success(
        `${count} song${count !== 1 ? "s" : ""} imported successfully!`,
      );
    },
    onError: (error) => {
...
export function useExportSetList() {
  return useMutation({
    ...
    onSuccess: async ({ setList, setListData }) => {
      ...
      if (result.success) {
        toast.success(
          `Exported to SetLists/${result.path || defaultFileName}`,
          {
            duration: 3000,
          },
        );
  ...
  return useMutation({
    mutationFn: async (setListFiles: SetListFileData[]) => {
      ...
      for (const setListData of setListFiles) {
        // Create the set list
        const createdId = await actor.createSetList(
          setListData.name,
          setListData.songIds,
        );
    ...
    onSuccess: (setListsToImport) => {
      ...
      toast.success(
        `${count} set list${count !== 1 ? "s" : ""} imported successfully!`,
      );
    },
    onError: (error) => {
