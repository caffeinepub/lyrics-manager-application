...
export default function SetListsTab({ onPlaySetList }: SetListsTabProps) {
  ...
  const handlePositionCommit = async (songId: string) => {
    ...
    const newPosition = Number.parseInt(inputValue, 10);
    ...
    if (targetIndex < 0 || targetIndex >= selectedSetList.songIds.length) {
      toast.error(
        `Position must be between 1 and ${selectedSetList.songIds.length}`,
      );
      setEditingPositions((prev) => {
        const newState = { ...prev };
        delete newState[songId];
  ...
  const handleMoveSong = async (songId: string, direction: "up" | "down") => {
    ...
    await moveSongMutation.mutateAsync({
      setListId: selectedSetListId,
      songId,
      direction,
    });
  ...
  const handleImportSetLists = () => {
    ...
    input.onchange = async (e) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          ...
          if (!data.name || !data.songIds || !Array.isArray(data.songIds)) {
            errors.push(
              `${file.name}: Missing required fields or invalid format`,
            );
            continue;
          }
      ...
      if (errors.length > 0) {
        toast.error(
          `Failed to import ${errors.length} file(s): ${errors.join(", ")}`,
        );
      }
      ...
      if (supportsFileSystemAccess) {
        try {
          ...
          for (const setList of setLists) {
            const sanitizedName = setList.name
          ...
          toast.success(
            `Successfully exported ${setLists.length} set list${setLists.length !== 1 ? "s" : ""}!`,
          );
        } catch (err: any) {
        ...
        toast.success(
          `Exported ${setLists.length} set list${setLists.length !== 1 ? "s" : ""}!`,
        );
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="sm" variant="outline" disabled={isExportingAll}>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={handleExportAllSetLists}
            ...
            {setLists.map((setList) => {
              ...
              if (isEditing) {
                return (
                  ...
                  >
                    <div className="space-y-2">
                      <Label htmlFor={`edit-name-${setList.id}`}>
                        Set List Name
                      </Label>
                      <Input
                    ...
                    <Dialog
                      open={isSongSelectionOpen}
                      onOpenChange={setIsSongSelectionOpen}
                    >
                      <DialogTrigger asChild>
                        <Button
                      ...
                      <DialogContent className="max-w-2xl max-h-[80vh]">
                        <DialogHeader>
                          <DialogTitle>
                            Select Songs for {editingName}
                          </DialogTitle>
                          <DialogDescription>
                            Check or uncheck songs to add or remove them. Type a
                            position number and press Enter to reorder. Use
                            arrow buttons to move songs up or down.
                          </DialogDescription>
                        </DialogHeader>
                        <ScrollArea className="h-[500px] pr-4">
                          {allSongs.length === 0 ? (
                            <div className="text-center py-12 text-muted-foreground">
                              <Music className="h-12 w-12 mx-auto mb-4" />
                              <p>
                                No songs available. Create some songs first!
                              </p>
                            </div>
                          ) : (
                            <div className="space-y-2">
                              ...
                              {allSongs
                                .filter(
                                  (song) =>
                                    !selectedSetList?.songIds.includes(song.id),
                                )
                                .map((song) => (
                                  <div
                                    key={song.id}
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <Download className="h-4 w-4 mr-2" />
                        {exportSetListMutation.isPending
                          ? "Exporting..."
                          : "Export"}
                      </Button>
                      <Button
